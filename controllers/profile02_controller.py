...
class ProfileController(Resource):
    def post(self):
        ...
        data = request.get_json(
            silent=True
        )  # 요청 본문에 JSON 데이터가 없거나 유효하지 않을 경우 예외를 발생시키지 않고 대신 None을 반환하도록 하는 옵션
        if not data:
        ...
        if data and "questionsList" in data and "job" in data:
            ...
            if not isinstance(questions_list, list):
                abort(
                    400, "400-08: questionsList는 리스트여야 합니다"
            ...
            for q in questions_list:
                ...
                for attribute in ("question_idx", "answer_idx", "type"):
                    if attribute not in q:
                        abort(
                            400, f"400-06: 각 질문은 '{attribute}'를 포함해야 합니다"
                ...
                try:
                    ...
                    abort(
                        400, "400-09: question_idx와 answer_idx는 정수여야 합니다"
            ...
            for i in range(1, 19):
                ...
                if key not in answer_data:
                    abort(
                        400, f"400-10: 질문 {i} 번에 대한 응답이 누락되었습니다"
            ...
            for i in range(1, 19):
                profile_parser.add_argument(
                    f"q{i}", type=int, required=True, help=f"질문 {i} 번에 대한 응답은 공백이 불가합니다"
                )
